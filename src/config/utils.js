...
export const getParentNode = (elem, name, type, inVal) => {
  ...
  while ((elem = elem['parentNode']) && elem.nodeType !== 9) {
    if (nodeType === 'class' && elem.nodeType === 1 && elem.classList.contains(name)) {
      matched.push(elem)
    }
    if (nodeType === 'id' && elem.nodeType === 1 && elem.id === name) {
      matched.push(elem)
    }
    if (nodeType === 'attribute' && elem.nodeType === 1 && elem.getAttribute(name)) {
      if (inVal) {
...
export const getChildNodes = (elem, name, type, inVal) => {
  ...
  for (let i = 0; i < elems.length; i++) {
    ...
    if (nodeType === 'class' && elemItem.nodeType === 1 && elemItem.classList.contains(name)) {
      matched.push(elemItem)
    }
    if (nodeType === 'id' && elemItem.nodeType === 1 && elemItem.id === name) {
      matched.push(elemItem)
    }
    if (nodeType === 'attribute' && elemItem.nodeType === 1 && elemItem.getAttribute(name)) {
      if (inVal) {
